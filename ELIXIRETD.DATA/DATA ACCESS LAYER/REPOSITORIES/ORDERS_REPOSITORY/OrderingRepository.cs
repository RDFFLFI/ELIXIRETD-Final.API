...
namespace ELIXIRETD.DATA.DATA_ACCESS_LAYER.REPOSITORIES.OrderingRepository
{
    public class OrderingRepository : IOrdering
    {
        ...
        public async Task<IReadOnlyList<TotalListOfApprovedPreparedDateDto>> TotalListOfApprovedPreparedDate(string customername)
        {
            ...
            var orders = _context.Orders
                 .GroupJoin(rushorder, order => order.OrderNoPKey, rushgenerate => rushgenerate.MIRId, (order, rushgenerate) => new { order, rushgenerate })
        ...
        public async Task<PagedList<GetAllListForMoveOrderPaginationDto>> GetAllListForMoveOrderPagination(UserParams userParams)
        {
           ...
           .GroupJoin(rushorder, order => order.OrderNoPKey, rushgenerate => rushgenerate.MIRId, (order, rushgenerate) => new { order, rushgenerate })
              .SelectMany(x => x.rushgenerate.DefaultIfEmpty(), (x, rushgenerate) => new { x.order, rushgenerate })
              .Where(x => x.order.IsActive == true && x.order.IsApproved == true && x.order.IsMove == false)
              .GroupBy(x => new
              {
                  x.order.CustomerName,
                  x.order.IsActive,
        ...
        public async Task<PagedList<ForApprovalMoveOrderPaginationDto>> ForApprovalMoveOrderPagination(UserParams userParams)
        {
            ...
            var order = _context.MoveOrders
                ...
                .Where(x => x.moveorder.IsApprove == null && x.moveorder.IsReject == null && x.moveorder.IsActive == true && x.moveorder.IsPrepared == true)
                .GroupBy(x => new
                {
                    x.moveorder.OrderNo,
        ...
        public async Task<PagedList<ApprovedMoveOrderPaginationDto>> ApprovedMoveOrderPagination(UserParams userParams)
        {
            ...
            var order = _context.MoveOrders
                 .GroupJoin(rushorder, moveorder => moveorder.OrderNo, rushgenerate => rushgenerate.MIRId, (moveorder, rushgenerate) => new { moveorder, rushgenerate })
                .SelectMany(x => x.rushgenerate.DefaultIfEmpty(), (x, rushgenerate) => new { x.moveorder, rushgenerate })
                .Where(x => x.moveorder.IsApprove == true)
                .GroupBy(x => new
                {
                    x.moveorder.OrderNo,
                    x.moveorder.CustomerName,
                    ...
                    x.moveorder.IsPrepared,
                    x.moveorder.IsReject,
                    x.moveorder.ApproveDateTempo,
                    x.moveorder.IsPrint,
        ...
        public async Task<PagedList<RejectedMoveOrderPaginationDto>> RejectedMoveOrderPagination(UserParams userParams)
        {
            ...
            var orders = _context.MoveOrders
                 .GroupJoin(rushorder, moveorder => moveorder.OrderNo, rushgenerate => rushgenerate.MIRId, (moveorder, rushgenerate) => new { moveorder, rushgenerate })
                .SelectMany(x => x.rushgenerate.DefaultIfEmpty(), (x, rushgenerate) => new { x.moveorder, rushgenerate })
                .Where(x => x.moveorder.IsApproveReject == true)
                .GroupBy(x => new
                {
                    x.moveorder.OrderNo,
                    ...
                    x.moveorder.OrderDate,
                    x.moveorder.PreparedDate,
                    x.moveorder.IsApprove,
                    x.moveorder.IsReject,
                    x.moveorder.RejectedDateTempo,
                    //x.Remarks,
