            ...
            public async Task<Unit> Handle(ConsolidateFinanceExportCommand command, CancellationToken cancellationToken)
            {
                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    for (var index = 1; index <= consolidate.Count; index++)
                    {
                        ...
                        row.Cell(11).Value = consolidate[index - 1].Reason;
                        row.Cell(12).Value = consolidate[index - 1].Reference;
                        row.Cell(13).Value = consolidate[index - 1].EncodedBy;
                        row.Cell(14).Value = consolidate[index - 1].CompanyCode;
                        row.Cell(15).Value = consolidate[index - 1].CompanyName;
                        row.Cell(16).Value = consolidate[index - 1].DepartmentCode;
                        row.Cell(17).Value = consolidate[index - 1].DepartmentName;
